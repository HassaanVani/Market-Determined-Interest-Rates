...
st.caption("Decentralized Credit, Peer-to-Peer Loan Generation, and Interest Rate Discovery")

# Sidebar for simulation configuration
st.sidebar.markdown("### Model Configuration")

# Agent counts
n_firms = st.sidebar.slider("Number of Firms", min_value=1, max_value=20, value=5, step=1)
n_banks = st.sidebar.slider("Number of Banks", min_value=1, max_value=5, value=2, step=1)
steps = st.sidebar.slider("Simulation Steps", min_value=2, max_value=50, value=15, step=1)

# Regulatory Parameters
st.sidebar.markdown("### Regulatory Policy")
reserve_req = st.sidebar.slider("Reserve Requirement Ratio", min_value=0.01, max_value=0.30, value=0.10, step=0.01)
capital_req = st.sidebar.slider("Basel Capital Ratio", min_value=0.01, max_value=0.20, value=0.08, step=0.01)
leverage_limit = st.sidebar.slider("Firm Leverage Limit", min_value=0.5, max_value=5.0, value=1.5, step=0.1)
...
llm_model = st.sidebar.selectbox("LLM Model", ["deepseek-r1:1.5b", "deepseek-r1:8b"], index=0)
agent_sentiment = st.sidebar.selectbox("Agent Sentiment Context", ["neutral", "optimistic", "pessimistic"], index=0)
llm_temperature = st.sidebar.slider("LLM Temperature", min_value=0.0, max_value=1.5, value=0.7, step=0.1)
...
if run_button:
    ...
    for s in range(1, steps + 1):
        ...
        m_inflation_metric.metric("Realized Inflation", f"{model.realized_inflation*100:.2f}%")
        m_rate_metric.metric("Real Interest Rate", f"{model.real_interest_rate*100:.2f}%")
        ...
        axs[0, 0].plot(history["step"], history["money_supply"], color='#6366F1', linewidth=2, label="Money Supply")
        axs[0, 0].plot(history["step"], history["outstanding_debt"], color='#F59E0B', linewidth=2, linestyle='--', label="Credit Debt")
        axs[0, 0].set_title("Money Supply & Credit Debt", color='#F8FAFC', fontsize=11, fontweight='semibold')
        axs[0, 0].legend(facecolor='#1E293B', edgecolor='#334155', loc="upper left")
        
        # 2. Interest Rates
        axs[0, 1].plot(history["step"], history["nominal_rate"], color='#3B82F6', linewidth=2, label="Nominal Rate")
        axs[0, 1].plot(history["step"], history["real_interest_rate"], color='#10B981', linewidth=2, label="Real Rate")
        axs[0, 1].set_title("Nominal vs. Real Interest Rates", color='#F8FAFC', fontsize=11, fontweight='semibold')
        axs[0, 1].legend(facecolor='#1E293B', edgecolor='#334155', loc="upper left")
        
        # 3. Inflation vs. Expectations
        axs[1, 0].plot(history["step"], history["realized_inflation"], color='#EF4444', linewidth=2, label="Realized Inflation")
        axs[1, 0].plot(history["step"], history["exp_inflation"], color='#8B5CF6', linewidth=2, linestyle=':', label="Expected Inflation")
        axs[1, 0].set_title("Realized vs. Expected Inflation", color='#F8FAFC', fontsize=11, fontweight='semibold')
        axs[1, 0].legend(facecolor='#1E293B', edgecolor='#334155', loc="upper left")
        
        # 4. Bank Defaults & Write-offs
        axs[1, 1].bar(history["step"], history["defaults"], color='#EC4899', alpha=0.7, label="Defaults Count")
        ax2 = axs[1, 1].twinx()
        ax2.plot(history["step"], np.cumsum(history["write_offs"]), color='#14B8A6', linewidth=2, label="Cum. Write-Offs")
        ax2.tick_params(colors='#94A3B8', labelsize=9)
        ax2.grid(False)
        axs[1, 1].set_title("Bank Default Volume & Write-offs", color='#F8FAFC', fontsize=11, fontweight='semibold')
    ...
    st.info("Configure variables in the sidebar and press 'Run Simulation' to start the execution.")
